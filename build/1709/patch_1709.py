...
# 1) DOCX wie per Doppelklick oeffnen
...
# 5) Dateiname-Feld suchen; bevorzugt AutomationId 1001 (Windows-Dateidialog)
$edit=$null
try {
...
if(-not $edit){
  $edit=Find-ByName $dlg @('Dateiname:','File name:') [Windows.Automation.ControlType]::Edit
}
if(-not $edit){
...
# 6) Optionales Ueberschreiben bestaetigen
Start-Sleep -Milliseconds 600
$overwrite=Wait-Until {
  $root=Get-Root
  $b=Find-ByName $root @('Ja','Yes') [Windows.Automation.ControlType]::Button
  return $b
} 1800
if($overwrite){ Invoke-El $overwrite | Out-Null }

# 7) Auf fertige PDF warten
$ok=Wait-Until {
...
def main():
    ...
    APP.with_name('patch_1709_report.txt').write_text(
        ...
        'PDF-Erzeugung klickt jetzt den sichtbaren Word-Befehl Als Adobe PDF speichern per Windows UI Automation.\n'
