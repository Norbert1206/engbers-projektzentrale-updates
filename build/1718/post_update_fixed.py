...
def main():
    try:
        patch = BASE / 'patch_1718_fixed.py'
        if not patch.exists():
            raise RuntimeError('patch_1718_fixed.py fehlt im Programmordner.')
        ns = runpy.run_path(str(patch), run_name='pz_patch_1718_fixed')
        fn = ns.get('main')
        if not callable(fn):
            raise RuntimeError('patch_1718_fixed.py enthaelt keine main()-Funktion.')
        rc = fn()
        if rc not in (None, 0):
            raise RuntimeError(f'patch_1718_fixed.py lieferte Rueckgabecode {rc}.')
