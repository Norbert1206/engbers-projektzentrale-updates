...
'''
helper=r'''    def _current_share_link():
...
if basket.count(anchor)!=1:
    raise RuntimeError("attachment helper anchor")
basket=basket.replace(anchor,helper+anchor,1)
...
new='''            body = tk.Text(frame, height=13, wrap="word", font=("Segoe UI", 10), bd=1, relief="solid")
            ...
            attachments = _attachment_files()
'''
basket=one(basket,old,new,"Outlook body insertion")
...
new='''            status_text = (
            ...
            status.pack(fill="x")
'''
basket=one(basket,old,new,"Outlook link status")
...
print("OK 1.8.36 Outlook HiDrive link insertion")
