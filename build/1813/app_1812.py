...
# PZ_FILE_RECYCLE_V1880: Dateien aus den Dateiansichten sicher in den
# Windows-Papierkorb verschieben. Ordner werden bewusst nicht angenommen.
def send_files_to_recycle_bin(paths):
    ...
    if not files:
        raise RuntimeError('Es wurde keine vorhandene Datei ausgewählt.')
    if os.name!='nt':
        raise RuntimeError('Der sichere Windows-Papierkorb ist auf diesem System nicht verfügbar.')
    import ctypes
    ...
    source='\0'.join(str(path) for path in files)+'\0\0'
    ...
    if result or operation.fAnyOperationsAborted:
        raise RuntimeError('Die Datei konnte nicht in den Windows-Papierkorb verschoben werden.')
    return files


def confirm_recycle_files(parent, paths, on_success=None, title='Dateien löschen'):
    files=[]; seen=set()
    for raw in paths or ():
        path=Path(raw)
        key=str(path).casefold()
        if key in seen or not path.exists() or not path.is_file():continue
        seen.add(key); files.append(path)
    if not files:
        messagebox.showwarning(title,'Bitte mindestens eine Datei auswählen.',parent=parent)
        return False
    names='\n'.join('• '+path.name for path in files[:12])
    if len(files)>12:names+=f'\n• … und {len(files)-12} weitere Datei(en)'
    if not messagebox.askyesno(
        title,
        f'{len(files)} Datei(en) in den Windows-Papierkorb verschieben?\n\n{names}\n\nOrdner werden nicht gelöscht.',
        parent=parent,
    ):
        return False
    try:
        removed=send_files_to_recycle_bin(files)
    except Exception as exc:
        messagebox.showerror(title,str(exc),parent=parent); return False
    if callable(on_success):
        try:on_success()
        except Exception:pass
    messagebox.showinfo(title,f'{len(removed)} Datei(en) wurden in den Windows-Papierkorb verschoben.',parent=parent)
    return True
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS commercial_texts(
      id INTEGER PRIMARY KEY, project_id INTEGER, title TEXT NOT NULL, category TEXT, body TEXT,
      is_global INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT
    );
    ''')
    ...
    cur.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version','5')")
...
class App(tk.Tk):
    ...
    def build_shell(self):
        ...
        nav=[('Übersicht',self.show_dashboard),('Projektakte',self.show_project),('Projekt-Explorer',self.show_project_explorer),('Statik',self.show_statics),('Wärmeschutz',self.show_thermal),('Schallschutz',self.show_acoustic),('Bauleitung',self.show_site),('Kommunikation',self.show_comm),('Aufgaben',self.show_tasks),('Prüfstatik',self.show_checker),('Angebote & Rechnungen',self.show_commercial),('Dokumente',self.show_documents),('PDF stempeln',self.show_pdf_stamp),('Schnittstellen',self.show_interfaces),('Sicherung / Update',self.show_backup)]
        compact_nav=self.winfo_screenheight()<=1100
        nav_pady=5 if compact_nav else 9
        subnav_pady=2 if compact_nav else 4
        for label,cmd in nav:
            b=tk.Button(self.sidebar,text=label,command=lambda l=label,c=cmd:self.navigate(l,c),anchor='w',bd=0,bg=DARK,fg='#eceae4',activebackground=DARK2,activeforeground='white',font=('Segoe UI',10),padx=24,pady=nav_pady,cursor='hand2')
            b.pack(fill='x'); self.nav_buttons[label]=b
            if label=='Statik':
                # PZ_STATIK_PDF_NAV_V1444
                # PZ_STATIK_PDF_NAV_V1445: eigener Statik-PDF-Bereich
                self.statik_pdf_nav=tk.Button(self.sidebar,text='   ↳ Statik PDF',command=self.show_statik_pdf,anchor='w',bd=0,bg=DARK,fg='#b9b7b0',activebackground=DARK2,activeforeground='white',font=('Segoe UI',9),padx=30,pady=subnav_pady,cursor='hand2')
    ...
    def new_project_wizard(self):
        ...
        def create():
            ...
            try:
                ...
                for folder in ['01_Projekt','02_Statik','03_Pruefstatik','04_Waermeschutz','05_DGNB_QNG','06_Schallschutz','07_Bauleitung','08_Kommunikation','09_Dokumente','10_Angebote_Rechnungen','Goodnotes']: (root/folder).mkdir(parents=True,exist_ok=True)
    ...
    def show_pdf_stamp(self):
        # PZ_PDF_STAMP_TOOL_V1870
        from pdf_stamp_v1870 import open_pdf_stamp_dialog
    ...
    def recycle_files(self, paths, on_success=None, title='Dateien löschen'):
        """Einheitlicher Papierkorb-Dialog fuer alle Dateiansichten."""
        return confirm_recycle_files(self, paths, on_success=on_success, title=title)
    ...
    def show_project_explorer(self):
        ...
        open_btn.configure(command=open_selected); explorer_btn.configure(command=show_in_explorer); copy_btn.configure(command=copy_path); delete_btn.configure(command=delete_selected_files)
        ...
        tr.bind('<Delete>',lambda e:(delete_selected_files(),'break')[1])
        render_normal()
    ...
    def show_statik_pdf(self):
        # PZ_PROJECT_BATCH_STAMP_V1870: projektbezogene Einzel- und Stapelstempelung.
        ...
        stamp_tools=tk.Frame(self.content,bg=BG); stamp_tools.pack(fill='x',pady=(0,10))
        ...
        def delete_selected_files():
            ...
            for iid in tree.selection():
                ...
                files.append(p)
            confirm_recycle_files(self,files,on_success=refresh,title='Statik PDF')
        ...
        def open_batch_stamp(profile):
            folder=find_folder()
            if not folder.exists():
                messagebox.showwarning(
                    'PDFs stempeln',
                    f'Der Statik-PDF-Ordner ist noch nicht vorhanden:\n\n{folder}',
                )
                return
            try:
                candidates=sorted(
                ...
                messagebox.showerror('PDFs stempeln',str(exc)); return
            selected=[]
            for iid in tree.selection():
                p=pathmap.get(iid)
                if p is not None and p.is_file() and p.suffix.casefold()=='.pdf':
                    selected.append(p)
            try:
                from project_batch_stamp_v1870 import open_project_batch_stamp_dialog
                open_project_batch_stamp_dialog(
                    self,
                    candidates,
                    profile=profile,
                    project_root=folder,
                    preselected_paths=selected,
                    on_complete=refresh,
                )
            except Exception as exc:
                messagebox.showerror('PDFs stempeln',str(exc))

        tk.Label(
            stamp_tools,
            text='PDFs direkt aus diesem Projekt auswählen und gesammelt stempeln:',
            bg=BG,fg=MUTED,anchor='w',
        ).pack(side='left')
        tk.Button(
            stamp_tools,text='STATIKBERECHNUNG STEMPELN',
            command=lambda:open_batch_stamp('statik'),
            bg=DARK,fg='white',bd=0,padx=13,pady=8,
        ).pack(side='right',padx=(6,0))
        tk.Button(
            stamp_tools,text='PLÄNE STEMPELN',
            command=lambda:open_batch_stamp('plan'),
            bg=ACCENT,fg='white',bd=0,padx=13,pady=8,
        ).pack(side='right',padx=(6,0))
        ...
        tree.bind('<Delete>',lambda _e:(delete_selected_files(),'break')[1])
...
"""
    ...
    def show_commercial(self):
        ...
        self.clear()
        try:
            from commercial_v1890 import render_commercial
            return render_commercial(self, self.content, DB_PATH)
        except Exception as exc:
            messagebox.showerror('Angebote & Rechnungen',str(exc),parent=self)
            self.show_project()
    ...
    def make_backup(self):
        ...
        with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as z:
            ...
            manifest={'app_version':APP_VERSION,'created':now_iso(),'schema_version':5,'project_root':str(PROJECT_ROOT)}
