...
MARKER = "PZ_PDF_STAMP_TOOL_V1870"
BASE = Path(__file__).resolve().parent
STAMP_META_PREFIX = "EngbersProjektzentraleStamp="
...
STAMP_DISPLAY_TO_KIND = {label: kind for kind, label in STAMP_LABELS.items()}
...
def pdf_stamp_info(pdf_path):
    """Return the Projektzentrale stamp marker stored in the PDF metadata."""
    fitz = _load_fitz()
    ...
    try:
        ...
        for item in keywords.split("|"):
            item = item.strip()
            if item.startswith(STAMP_META_PREFIX):
                return item
        return ""
    finally:
        document.close()
...
def _write_stamp_metadata(document, profile: str, stamp_kind: str):
    current = document.metadata or {}
    allowed = (
        "title", "author", "subject", "keywords", "creator", "producer",
        "creationDate", "modDate", "trapped",
    )
    metadata = {key: str(current.get(key) or "") for key in allowed}
    existing = metadata.get("keywords", "").strip()
    marker = (
        STAMP_META_PREFIX
        + f"profile:{profile};kind:{stamp_kind};time:"
        + _dt.datetime.now().astimezone().isoformat(timespec="seconds")
    )
    metadata["keywords"] = (existing + " | " + marker).strip(" |")
    metadata["producer"] = "Engbers Projektzentrale " + MODULE_VERSION
...
def stamp_pdf(
    ...
    try:
        ...
        existing_stamp = ""
        keywords = str((document.metadata or {}).get("keywords") or "")
        for item in keywords.split("|"):
            if item.strip().startswith(STAMP_META_PREFIX):
                existing_stamp = item.strip()
                break
        if existing_stamp and not allow_existing_stamp:
            raise RuntimeError(
                "Diese PDF wurde bereits mit der Projektzentrale gestempelt. "
                "Ein zweiter Stempel wurde verhindert."
            )
        ...
        _write_stamp_metadata(document, profile, stamp_kind)
...
class PdfStampDialog(tk.Toplevel):
    def __init__(self, parent=None, initial_pdf=None, initial_profile="statik"):
        ...
        self.stamp_var = tk.StringVar(value=STAMP_LABELS["statik"])
    ...
    def _build_ui(self):
        ...
        self.stamp_combo = ttk.Combobox(
            ...
            values=(STAMP_LABELS["statik"], STAMP_LABELS["sasv"]),
            width=25,
        )
    ...
    def _current_stamp_kind(self):
        selected = STAMP_DISPLAY_TO_KIND.get(self.stamp_var.get(), "statik")
        return _profile_stamp_kind(self.profile_var.get(), selected)
    ...
    def _apply(self):
        ...
        try:
            ...
            result = stamp_pdf(
                ...
                stamp_kind=STAMP_DISPLAY_TO_KIND.get(self.stamp_var.get(), "statik"),
