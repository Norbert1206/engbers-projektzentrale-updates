...
OLD='1.7.15'
NEW='1.7.16'
MARK='PZ_PDF_SAVE_KEYS_V1716'

NEW_FUNC=r'''def _export_pdf_with_word(docx_path, pdf_path):
    # PZ_PDF_SAVE_KEYS_V1716
    # PDF-Druck wie bisher, aber der Windows-Speichern-Dialog wird nur noch per
    # Tastatur bedient: Dialog aktivieren -> Strg+L Ordner -> Alt+N Dateiname -> Enter.
...
public static class EngbersSaveWin32 {
...
function Activate-Dialog($w){
  ...
  try {
    ...
    if($h -ne [IntPtr]::Zero){
      try { [EngbersSaveWin32]::ShowWindowAsync($h,9) | Out-Null } catch {}
      try { [EngbersSaveWin32]::SetForegroundWindow($h) | Out-Null } catch {}
...
$PdfDir=[IO.Path]::GetDirectoryName($PdfPath)
$PdfName=[IO.Path]::GetFileName($PdfPath)
$oldDefault=$null
...
try {
  ...
  # 1) Richtigen Zielordner setzen. Damit ist der zuletzt benutzte Windows-Ordner egal.
  ...
  # 2) Dateiname-Feld ueber den Windows-Standard-Hotkey aktivieren.
...
def _set_version(text):
    ...
    if n!=1: raise RuntimeError('1.7.16: APP_VERSION wurde in app.py nicht gefunden.')
    return out.replace('Projektzentrale 1.7.15','Projektzentrale 1.7.16')
...
def _replace_function(source,name,new_block):
    ...
    raise RuntimeError(f'1.7.16: Funktion {name} wurde nicht gefunden.')


def main():
    if not APP.exists() or not MOD.exists(): raise RuntimeError('1.7.16: app.py oder wordforms_v1700.py wurde nicht gefunden.')
    app=APP.read_text(encoding='utf-8'); mod=MOD.read_text(encoding='utf-8'); cur=_version(app)
    if cur==NEW and MARK in mod: return 0
    if cur!=OLD: raise RuntimeError(f'Update 1.7.16 erwartet Projektzentrale {OLD}; gefunden: {cur or "unbekannt"}.')
    app_new=_set_version(app); mod_new=_replace_function(mod,'_export_pdf_with_word',NEW_FUNC)
    if MARK not in mod_new: raise RuntimeError('1.7.16: Tastatur-Speicherdialog-Marker fehlt nach dem Patch.')
    ...
    for required in ("SendWait('^l')","SendWait('%n')",'Paste-Text $PdfDir','Paste-Text $PdfName','Microsoft Print to PDF'):
        if required not in active: raise RuntimeError('1.7.16: erwarteter Tastatur-Baustein fehlt: '+required)
    if 'FileNameControlHost' in active or 'Find-FileNameEdit' in active:
        raise RuntimeError('1.7.16: alte UIAutomation-Dateiname-Erkennung ist noch aktiv.')
    chk1=APP.with_name('app.py.1716.check'); chk2=MOD.with_name('wordforms_v1700.py.1716.check')
    try:
    ...
    for p in (APP,MOD): shutil.copy2(p,p.with_name(p.name+'.vor_1_7_16_'+stamp+'.bak'))
    t=APP.with_name('app.py.1716.tmp'); t.write_text(app_new,encoding='utf-8'); t.replace(APP)
    t=MOD.with_name('wordforms_v1700.py.1716.tmp'); t.write_text(mod_new,encoding='utf-8'); t.replace(MOD)
    APP.with_name('patch_1716_report.txt').write_text('OK: Projektzentrale 1.7.16 installiert.\nPDF-Speicherdialog wird jetzt direkt per Tastatur auf Zielordner und Dateiname gesetzt.\n',encoding='utf-8')
    return 0
...
if __name__=='__main__':
    try: raise SystemExit(main())
        ...
        try: APP.with_name('patch_1716_error.txt').write_text(traceback.format_exc(),encoding='utf-8')
        except Exception: pass
        raise
